...
//	@IBOutlet weak var btnMenu: UIButton!
	...
	override func viewDidLoad() {
		...
		NotificationCenter.default.addObserver(self, selector: #selector(TabBarViewController.showMapBecauseOfNotification), name:NSNotification.Name(rawValue: "openedViaNotification"), object: nil)
		NotificationCenter.default.addObserver(self, selector: #selector(TabBarViewController.handlePush), name:NSNotification.Name(rawValue: "showMapBecauseOfHandleNotification"), object: nil)
		
		profilePic.layer.cornerRadius = 0.5 * profilePic.bounds.size.width
		profilePic.clipsToBounds = true
		
		tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(TabBarViewController.closeSidebar))
		recognizer = UIScreenEdgePanGestureRecognizer(target: self, action:#selector(TabBarViewController.openSidebarGesture(_:)))
		let closeRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(TabBarViewController.closeSidebar))
	...
	func animateSwipeRight() {
		...
		if tutorial.swipeToOpenMenu == false {
		_ = Timer.scheduledTimer(timeInterval: 3, target: self, selector: #selector(TabBarViewController.animateSwipeRight), userInfo: nil, repeats: false)
		}
	}
	...
	func animateGroupsButton() {
		...
		if tutorial.groupsButton == false {
			_ = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(TabBarViewController.animateGroupsButton), userInfo: nil, repeats: false)
		} else {
