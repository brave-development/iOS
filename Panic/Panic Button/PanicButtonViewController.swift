...
class AlertButtonViewController: UIViewController, UIGestureRecognizerDelegate, UITextViewDelegate {
...
//    var manager : CLLocationManager!
    ...
    override func viewDidLoad() {
        ...
        btnAlert.backgroundColor = UIColor(white: 0, alpha: 0.4)
    ...
    func showActivationConfirmation() {
        let saveAlert = UIAlertController(title: NSLocalizedString("activate", value: "Activate", comment: "confirmation to activate the Alert button"), message: NSLocalizedString("activate_confirmation_text", value: "Activate Brave and send notifications?", comment: ""), preferredStyle: UIAlertControllerStyle.alert)
    ...
    func activateAlert() {
        self.showDetailsInput {
            ...
            self.activate_UIChanges()
            alertHandler.startAlert(drug: drug) {
                success in
                ...
                if success {
                    ...
                    self.btnChat.sendActions(for: .touchUpInside)
                    print("ALERT SENT")
                } else {
                    self.deativate_UIChanges()
    ...
    func showDetailsInput(completionHandler:@escaping (_ drug: String)->Void) {
        ...
        let textView = SZTextView(frame: CGRect(x: 0, y: 0, width: 210, height: 40))
...
//        })
    ...
    func changeButtonStyle(to style: AlertButtonStyle) {
        switch style {
        case .activate:
            btnAlert.setTitle(NSLocalizedString("deactivate", value: "Cancel", comment: ""), for: UIControlState())
